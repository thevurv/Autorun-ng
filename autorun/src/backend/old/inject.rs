use std::{os::raw::{c_void, c_char, c_int}, sync::Mutex, mem::MaybeUninit};
use crate::backend::error::InjectError;
...
impl Autorun {
	pub fn inject(&mut self) -> Result<(), error::InjectError> {
		...
		if let Some(process) = gmod {
			...
			match bin_folder.file_name() {
				Some(x) if x != "win64" && x != "bin" && x != "linux32" && x != "linux64" => {
					Err(error::InjectError::WeirdPath)
				},
				_ => {
			...
			Err(error::InjectError::GameNotFound)
		}
	}
	...
	/// wtf
	pub fn uninject(&mut self) -> Result<(), error::InjectError> {
