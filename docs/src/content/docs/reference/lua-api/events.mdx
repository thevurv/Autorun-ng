...
export function getRealmEmoji(realm) {
    if (realm === 'shared') return '🟢🟠';
    if (realm === 'menu') return '🟢';
    if (realm === 'client') return '🟠';
    return '';
}

<Aside type="note">
Events can be listened to using `Autorun.on(eventName, callback)`. The callback's return value can modify event behavior.
</Aside>

{apiData.events && apiData.events.map(event => (
  <div key={event.name}>
    <h2>{getRealmEmoji(event.realm)} <code>{event.name}</code></h2>
