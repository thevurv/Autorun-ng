...
testDetour = Autorun.detour(_G.game.GetIPAddress, function(orig, ...)
    ...
    _G.PrintTable(args)
    _G.timer.Simple(5, function()
        _G.print("5 seconds passed since detour call.")
        Autorun.detour_remove(testDetour)
    end)
    return orig()
end)

Autorun.print("Detour test: " .. tostring(testDetour))
local orig = Autorun.get_original_function(testDetour)
Autorun.print("Original function: " .. tostring(orig))
Autorun.print("IP address from original function: " .. tostring(orig()))